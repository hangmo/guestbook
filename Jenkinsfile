...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="hangmo/cicd_guestbook:${strDockerTag}"
    }
    ...
    stages {
        stage('Checkout') {
            agent { label 'agent1' }
            steps {
                git branch: 'master', url:'https://github.com/hangmo/guestbook.git'
            }
        }
        stage('Build') {
            agent { label 'agent1' }
    ...
    post { 
        always { 
            emailext (attachLog: true, body: '본문', compressLog: true
                    , recipientProviders: [buildUser()], subject: '제목', to: 'hangmo.j@gmail.com')
